...
from .plotting_mse_running_min_growth import (
    _infer_print_freq,
...
def _plot_eval_G_multi_on_ax(
    ...
    """
    Same logic as plot_eval_G_multi, but:
      - draws into an existing axis `ax`
      - uses wrapper.growth_preds[frame_idx] to animate G(u) over epochs.
    """
    if errs:
        err_up, err_low = errs
    else:
    ...
    for wrapper_dic, color, label in zip(modelWrapper_dics.values(), colors, labels):
        ...
        for wrapper in wrapper_dic.values():
            # true G (does not depend on epoch)
            G_true_check = list(wrapper.model.G_true)
            if G_true_check not in G_true_lst:
                G_true_lst.append(G_true_check)

            # --- epoch-dependent prediction from growth_preds ---
            preds = wrapper.growth_preds
            if len(preds) == 0:
                continue

            # clip frame_idx just in case some runs are shorter
            idx = min(frame_idx, len(preds) - 1)
            grow_pred = np.asarray(preds[idx].detach().numpy()).flatten()  # shape [N_u]

            if grow_pred.shape[0] != u_vals_np.shape[0]:
                raise ValueError(
                    "wrapper.growth_preds entry length does not match u_vals grid."
                )

            G_ensemble.append(grow_pred[None, :])  # [1, N_u]

        if not G_ensemble:
            # no predictions for this config at this frame
            continue

        G_ensemble = np.concatenate(G_ensemble, axis=0)  # [runs, N_u]
        G_mean = G_ensemble.mean(axis=0)
        G_min = G_ensemble.min(axis=0)
        G_max = G_ensemble.max(axis=0)

        # we don't really need MSE here for plotting
        results.append((label, color, G_mean, G_min, G_max))

    # ------------------------------------------------
    # Plot on given axis
    # ------------------------------------------------
    # Ensemble predictions for each configuration
    for j, (label, color, G_mean, G_min, G_max) in enumerate(results):
    ...
    ax.set_xlabel(r"cell density [days$^{-1}$]$", fontsize=11)
...
def save_gif_running_min_MSE_grow_loss_broken_x_log(
    models_dics_list,
    plot_params=None,
    plot_settings=None,
    out_path="running_min_grow_loss_broken_xaxis_loglog.gif",
    stride=1,
    dpi=120,
    fps=6,
    ...
    """
    Save an animated GIF of the running-min growth MSE loss for multiple
    experiment groups, using the same broken log–log x-axis layout as
    `plot_running_min_MSE_grow_loss_broken_x_log_lst`, and (optionally)
    a real-space G(u) subplot on the right.
    ...
    Right: growth G(u) in real space (cell density vs growth).
    """
    import imageio.v2 as imageio  # lazy import

    # ---------- Settings ----------
    merged = _merge_grow_plot_settings(plot_settings)
    xaxis = merged["xaxis"]
    ...
    # ---------- Getermine fixed real-space axis limits if not provided ----------
    do_realspace = (
    ...
    if do_realspace and (realspace_xlim is None or realspace_ylim is None):
        ...
        for wrapper_dic in modelWrapper_dics.values():
            for wrapper in wrapper_dic.values():
                ...
                # all predicted growth curves across epochs
    ...
    for k in range(1, max_len + 1, step):
        ...
        # --- FIXEG BACKGROP: plot full runs on loss axes (no truncation) ---   ### CHANGEG
        for run in all_run_data:
            x = run["_x_plot"]
            if len(x) == 0:
                continue
            ...
            # --- Best-model marker (static, same on every frame) ---            ### CHANGEG
        ...
        if current_epochs:
            ...
            # Graw cursor on the appropriate panel
        ...
        # --- Real-space G(u) subplot (if info provided) ---
        if do_realspace:
            frame_idx = k - 1  # 0-based index into wrapper.growth_preds
