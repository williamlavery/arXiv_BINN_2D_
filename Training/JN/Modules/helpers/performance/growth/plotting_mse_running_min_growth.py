...
def _default_grow_plot_settings():
    """Return default settings for the growth MSE plot."""
    return {
        ...
        "legend": {
            ...
            "fontsize": 10,
            "ncols": 1,
            "title": "$N_G$",   # legend title for lines
        },
        "name": "running_min_grow_loss_broken_xaxis_loglog.png",
        "fill": True,
...
def _merge_grow_plot_settings(plot_settings):
    """Merge user-provided settings with defaults and normalize structure."""
    defaults = _default_grow_plot_settings()
...
def _gather_all_run_data(models_dics_list, plot_params, marker_styles, line_styles):
    ...
    for i, models_dics in enumerate(models_dics_list):
        group_runs = prepare_grow_run_data(models_dics, plot_params=plot_params)
...
def _plot_growth_runs(
    ...
    for run in all_run_data:
        ...
        # Running-min stats for growth MSE
...
def _format_broken_x_axes(ax1, ax2, xaxis, fontsizes):
    ...
    ax1.set_ylabel(r"Growth MSE [mm$^4$ day$^{-2}$]", fontsize=yfont)
...
def _add_line_legend(ax1, ax2, label_to_color, legend, line_width):
    ...
    legend_title = legend.get("title", "$N_G$")
...
def plot_running_min_MSE_grow_loss_broken_x_log_lst(
    models_dics_list,
    plot_params=None,
    plot_settings=None,
):
    """
    Plot running-min growth MSE loss for multiple experiment groups,
