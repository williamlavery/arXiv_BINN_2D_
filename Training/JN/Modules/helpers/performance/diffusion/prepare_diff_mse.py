...
def prepare_diff_run_data(models_dics, plot_params=None, best_epoch_termination=True, val_loss_best=True):
    import numpy as np
    try:
        import torch
    except ImportError:
    ...
    for i, (key, model_dic) in enumerate(zip(model_keys, dic_values)):
        ...
        for m in model_wrappers:
            ...
            # Use full length of recorded diffusion errors for this model
            series_len = len(m.diffusion_errors)
        ...
        # ---------- Second pass: build padded diffusion error arrays ----------
        for m in model_wrappers:
            series_vals = []
            ...
            for diff_err in m.diffusion_errors:
                ...
                try:
                    if torch is not None and isinstance(diff_err, torch.Tensor):
                        val = torch.mean(diff_err[:, 0]).detach().cpu().numpy()
                    else:
                        arr = np.asarray(diff_err)
                        if arr.ndim > 1:
                            val = np.mean(arr[:, 0])
                        else:
                            val = np.mean(arr)
        ...
        running_mins = np.minimum.accumulate(
            np.nan_to_num(diff_errs_arr, nan=np.inf),
            axis=1,
        )
        ...
        if val_loss_best:
            ...
            best_seed_idx = np.nanargmin(diff_errs_arr[:, best_epoch_idx]) # CHECK this is correct
        ...
        # ---------- Final diffusion MSE stats (matching prepare_diffusion_mse_data) ----------
        # Each best_diffusion_error can be an array/tensor; take its mean per run
        final_mses = np.array([m.best_diffusion_error for m in model_wrappers], dtype=np.float64)
        ...
        run_data.append(
            {
                # --- scalar summary (from prepare_diffusion_mse_data) ---
